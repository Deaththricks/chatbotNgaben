...
Consolidates:
  - Graphing/kb/glossary_draft_review_batch2.md's "Confident drafts" (facts-based
    drafts, hand-curated)
  - Graphing/kb/tuning/definitions.json (LLM-paraphrased drafts, gated by
    definition_quality.py)
  - Graphing/kb/output/facts.jsonl (supporting facts, via kb_content.facts_for(), for
    entities with no draft at all -- something to write from)

into Graphing/kb/curation/glossary_review_master.md, tiered by how much material
exists, so a full pass over every undefined entity is one scroll instead of four
files. Supersedes batch2/batch3 as the single review surface going forward; batch2/
batch3 are left on disk (already-checked boxes in batch2 aren't lost) but this
file is now the one to work through.

Usage (from Graphing/kb/curation/, using the bot's venv so kb_resolver's deps resolve):
...
sys.path.insert(0, str(HERE.parent.parent.parent / "Chatbot" / "bot"))
...
ENTITIES_JSON = HERE.parent / "output" / "entities.json"
DEFINITIONS_JSON = HERE.parent / "tuning" / "definitions.json"
...
def main() -> None:
    ...
    batch2_raw = BATCH2_MD.read_text(encoding="utf-8") if BATCH2_MD.exists() else ""
    ...
    out = [
        ...
        f"entities (out of {len(entities)} total), in one place: draft text from "
        "`glossary_draft_review_batch2.md` and/or `Graphing/kb/tuning/definitions.json` "
        ...
        "",
        "**How to apply an approved definition:** add it as a "
        "`* Term:  Definition.` line to `Graphing/data/ngaben-glossary.txt` "
        "(same as batch 1), then rebuild "
